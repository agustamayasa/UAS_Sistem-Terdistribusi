                            ...
                            <form action="{{ route('batik.index') }}" method="GET">
                                <div class="flex">
                                    ...
                                    <div class="flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-gray-900 bg-gray-100 border border-gray-300 rounded-s-lg hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-700 dark:text-white dark:border-gray-600"
                                        type="button">
                                        <select class="text-gray-900 bg-gray-100" name="id_penjual" id="id_penjual">
                                            <option value="">Pilih Penjual</option>
                                            @foreach($seller as $item)
                                            <option value="{{ $item->id_penjual }}"
                                                {{ (isset($_GET['id_penjual'])&&$_GET['id_penjual']==$item->id_penjual)?'selected' : ''}}>
                                                {{$item->nama_penjual}}
                                            </option>
                                            @endforeach
                                        </select>
                ...
                <table class="mt-8 w-full border-collapse border border-gray-400 table-auto">
                    <thead>
                        <tr>
                            ...
                            <th class="border border-gray-200 px-4 py-2">Nama Kemeja</th>
                            <th class="border border-gray-200 px-4 py-2">Deskripsi</th>
                            <th class="border border-gray-200 px-4 py-2">Kode</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="border border-gray-200 px-4 py-2">
                                <img src="{{asset ($item->gambar_kemeja) }}" alt=""
                                    class="w-full md:h-[60px] md:w-[60px]  h-48 inset-0 object-cover object-top rounded-lg"
                                    loading="lazy" />
                            </td>
                            <td class="border border-gray-200 px-4 py-2">{{ $item->nama_kemeja }}</td>
                            <td class="border border-gray-200 px-4 py-2">{{ $item->deskripsi_kemeja }}</td>
                            <td class="border border-gray-200 px-4 py-2">{{ $item->kode_kemeja }}</td>
                            <td class="border border-gray-200 px-4 py-2">{{ $item->produk->nama_penjual}}</td>
                            <td class="border border-gray-200 px-4 py-2">{{ $item->harga }}</td>
                            <td class="border border-gray-200 px-4 py-2">
                                <span class="flex">
                                    <a href="{{ route('batik.edit', $item->id_kemeja) }}" <button
                                    ...
                                    <form action="{{ route('batik.destroy', $item->id_kemeja) }}" method="POST"
