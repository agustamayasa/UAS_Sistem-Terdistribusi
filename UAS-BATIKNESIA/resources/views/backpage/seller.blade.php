                    ...
                    <div class="w-1/4 ">
                        <span>
                            <a href="{{route('seller.create')}}">
                                <button type="button" id="createSellerButton" data-modal-toggle="createSellerModal"
                ...
                <table class="mt-8 w-full border-collapse border border-gray-400 table-auto">
                    <thead>
                        <tr>
                            ...
                            <th class="border border-gray-200 px-4 py-2">Nama Seller</th>
                            <th class="border border-gray-200 px-4 py-2">Deskripsi</th>
                            <th class="border border-gray-200 px-4 py-2">Kontak</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="border border-gray-200 px-4 py-2">
                                <img src="{{ asset($seller->logo_penjual) }}" alt=""
                                    class="w-full md:h-[60px] md:w-[60px]  h-48 inset-0 object-cover object-top rounded-lg"
                                    loading="lazy" />
                            </td>
                            <td class="border border-gray-200 px-4 py-2">{{ $seller->nama_penjual }}</td>
                            <td class="border border-gray-200 px-4 py-2">{{ $seller->deskripsi_penjual }}</td>
                            <td class="border border-gray-200 px-4 py-2">{{ $seller->kontak }}</td>
                            <td class="border border-gray-200 px-4 py-2">{{ $seller->alamat }}</td>
                            <td class="border border-gray-200 px-4 py-2">
                                <span class="flex">
                                    <a href="{{ route('seller.edit', $seller->id_penjual) }}" <button
                                    ...
                                    <form action="{{ route('seller.destroy', $seller->id_penjual) }}" method="POST"
                                        class="ml-2 flex items-center font-medium text-red-600 dark:text-sky-500 hover:underline mr-2">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                                            onclick="return confirm('Apakah anda yakin ingin menghapus data Seller?')"
